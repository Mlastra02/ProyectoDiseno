...
export default function Login() {
  ...
  const handleSubmit = async (e) => {
    ...
    if (res.ok) {
      ...
      alert(language === 'es' ? "Error en la autenticación. Inténtalo de nuevo." : "Authentication error. Please try again.");
    }
  };
    ...
    <div className="min-h-screen bg-gradient-to-br from-blue-900 to-blue-700 flex items-center justify-center p-6">
      <div className="w-full max-w-md bg-blue-800 bg-opacity-80 p-8 rounded-lg shadow-lg">
        ...
        <div className="mb-4 text-center">
          <label htmlFor="language" className="text-white mr-2">{language === 'es' ? 'Idioma' : 'Language'}:</label>
          <select
            id="language"
            value={language}
        ...
        <form onSubmit={handleSubmit} className="space-y-6">
          <div className="relative">
            <label htmlFor="username" className="absolute left-3 top-2 text-yellow-300">
          ...
          <div className="relative">
            <label htmlFor="password" className="absolute left-3 top-2 text-yellow-300">
              <svg xmlns="http://www.w3.org/2000/svg" className="h-5 w-5" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                <path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M12 4v4m0 4v8m4-4H8" />
              </svg>
            </label>
            <input
              type="password"
